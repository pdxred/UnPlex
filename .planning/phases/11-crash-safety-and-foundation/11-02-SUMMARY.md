...
plan: 02
subsystem: ui
tags: [brightscript, scenegraph, roku, refactor, deduplication, constants]

# Dependency graph
requires:
  - phase: 11-crash-safety-and-foundation/11-01
    provides: All 7 screens re-enabled with LoadingSpinner; crash-safe foundation
provides:
...
affects:
  ...
  - All future screen work that handles ratingKey values

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "ratingKey coercion: always call GetRatingKeyStr(item.ratingKey) from utils.brs — never inline type-check"
    - "Constants usage: use m.constants.POSTER_WIDTH (not hardcoded 240) for progress bar width calculations"
...
requirements-completed: [SAFE-03, FIX-07]
